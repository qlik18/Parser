...
namespace Logic.Implementation
{
    public class JiraIssues
    {
        ...
        private bool LoginJira()
        {
            try
            {
                jira = Jira.CreateRestClient("http://jira", login, password);
...
//                        taskUser = jira.GetUserAsync(iss.Reporter);
                ...
                for (int i = 0; i < iss.CustomFields.Count; i++)
                {
                    ...
                    try
                    {
                        //if (a.Id == "customfield_14355") // Id Konta
                        //{
                        //    konto.Add(a.Values[0]);
                        //}
                        if (a.Id == "customfield_14354") //Id Kontraktu
                        {
                            kontrakt.Add(a.Values[0]);
                        }
                        if (a.Id == "customfield_14350") //Rodzaj błędu
                        {
                            rblad.Add(a.Values[0]);
                        }
                        if (a.Id == "customfield_21150" ) // Rodzaj błędu ZO
                        {
                            rblad.Add(a.Values[0]);
                        }
                        if (a.Id == "customfield_19351") // Rodzaj błędu Problem
                        {
                            rblad.Add(a.Values[0]);
                        }
                        //if (a.Id == "customfield_12057")// Id zamówienia
                        //{
                        //    zamowienie.Add(a.Values[0]);
                        //}
                        if (a.Id == "customfield_14650") // oryginalne id
                        {
                            oryginalId.Add(a.Values[0]);
                        }
                        if (a.Id == "customfield_11450") // Opis rozszerzony 
                        {
                            opisRozszerzony.Add(a.Values[0]);
                        }
                        if (a.Id == "customfield_19350") // Środowisko problemu 
                        {
                            srodowiskoProblemuCF.Add(a.Values[0]);
                        }
                        if (a.Id == "customfield_20350") // Środowisko problemu 
                        {
                            /*
                            id Konta        - Wstrzymuje testy
                            Id zamówienia   - Wdrożenie priorytetowe
                            */
                            //if(a.Values[0] == "Wstrzymuje testy")// id Konta
                            //{
                            //    konto.Add(a.Values[0]);
                            //}
                            //if (a.Values.Count() == 2)
                ...
                //if (iss.Project == "CP")
                //{
                //    if (iss.Components.Count > 0)
                //    {
                //        rblad = new List<string>();
                //        rblad.Add(iss.Components[0].Name);
                //    }
                //}
        ...
        public IEnumerable<Issue> GetIssues(JiraProject project, bool przydzielone, string filterName = "")
        {
            //if (!LoginJira())
            //    return null;
            ...
            //        try
            //        {
            //            return await jira.Issues.GetIssuesFromFilter(filterName);
            //        }
            //        catch(InvalidOperationException ex)
            //        {
            //            List<Issue> emptyResult = new List<Issue>();
            ...
            //                }
            //                else
            //                    issues = await jira.Issues.GetIssuesFromFilter("nieprzydzielone");
